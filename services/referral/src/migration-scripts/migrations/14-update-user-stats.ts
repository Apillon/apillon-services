    ...
    CREATE OR REPLACE VIEW \`v_userStats\` AS
      ...
      FROM (
        ...
        LEFT JOIN (
          ...
          JOIN ${databases.storageDb}.deployment d
          ON w.id = d.website_id
          WHERE w.status = 5
          AND d.deploymentStatus = 10
          AND (d.environment = 2 OR d.environment = 3)
          GROUP BY project_uuid
        ) as www
...
export async function downgrade(
  ...
  // Previous version from 13-view-user-stats.ts
  await queryFn(`
    CREATE OR REPLACE VIEW \`v_userStats\` AS
      SELECT
        email, user_uuid, COUNT(project_uuid) as project_count, JSON_ARRAYAGG(project_uuid) as project_uuids,
