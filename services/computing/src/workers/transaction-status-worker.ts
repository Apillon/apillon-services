    ...
    await runWithWorkers(
      ...
      async (res: TransactionWebhookDataDto, ctx) => {
        ...
        if (transaction.refId) {
          ...
          switch (transaction.refTable) {
            case DbTables.CONTRACT: {
              ...
              if (!contract.exists()) {
                await this.writeEventLog({
                  logType: LogType.ERROR,
                  message: `Computing contract with id ${transaction.refId} not found.`,
                  service: ServiceName.COMPUTING,
                  data: {
                    transaction_id: transaction.id,
                    contract_id: transaction.refId,
                  },
                });
                return;
              }
            ...
            case DbTables.ACURAST_JOB: {
              const acurastJob = await new AcurastJob(
                {},
                this.context,
              ).populateById(transaction.refId);
              if (!acurastJob.exists()) {
                await this.writeEventLog({
                  logType: LogType.ERROR,
                  message: `Acurast job  with id ${transaction.refId} not found.`,
                  service: ServiceName.COMPUTING,
                  data: {
                    transaction_id: transaction.id,
                    contract_id: transaction.refId,
              ...
              if (transaction.transactionType === TransactionType.DEPLOY_JOB) {
                acurastJob.jobStatus = AcurastJobStatus.DEPLOYED;
                acurastJob.jobId = parseInt(res.data);
                acurastJob.transactionHash = transaction.transactionHash;
                await acurastJob.update();
                await this.writeEventLog({
                  logType: LogType.INFO,
                  project_uuid: acurastJob?.project_uuid,
                  message: `Status updated to ${acurastJob.jobStatus} for job with uuid ${acurastJob.job_uuid}.`,
                  service: ServiceName.COMPUTING,
                  data: {
                    job_uuid: acurastJob.job_uuid,
                    jobStatus: acurastJob.jobStatus,
                    updateTime: acurastJob.updateTime,
                  },
                });
              }
              break;
            ...
            default:
              throw new Error(`refTable ${transaction.refTable} not supported`);
          }
        } else if (
