...
export class PhalaDockerHostingStrategy implements IBackendHostingStrategy {
  ...
  async deployDockerCompose(
    ...
    if (pod.nodes.length === 0) {
      throw new HostingCodeException({
        status: 500,
        code: HostingErrorCode.HOSTING_POD_NOT_FOUND,
    ...
    try {
      ...
      throw new HostingCodeException({
        status: 500,
        code: HostingErrorCode.HOSTING_DEPLOY_FAILED,
        errorMessage,
      });
    }
  }
  ...
  async resizeInstance(
    ...
    try {
      ...
      throw new HostingCodeException({
        status: 500,
        code: HostingErrorCode.HOSTING_RESIZE_FAILED,
