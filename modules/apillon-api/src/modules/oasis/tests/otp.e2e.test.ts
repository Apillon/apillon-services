...
describe('OTP tests', () => {
  ...
  describe('OTP Generation tests', () => {
    ...
    test('User cannot create OTP without valid token', async () => {
      let response = await request(stage.http).post('/oasis/otp').send({
        email: 'test@apillon.io',
      });

      expect(response.status).toBe(400);

      response = await request(stage.http).post('/oasis/otp').send({
        email: 'test@apillon.io',
        token: 'invalid token',
      });
    ...
    test('Successfully create OTP', async () => {
      ...
      const response = await request(stage.http).post('/oasis/otp').send({
        email,
        token,
      });
  ...
  describe('OTP Validation tests', () => {
    ...
    test('User cannot validate OPT without valid token', async () => {
      ...
      response = await request(stage.http).post('/oasis/otp/validate').send({
        email,
        code: correctCode,
        token: 'invalid token',
      });
